...
func Parse(tokenString string,) (*Context, error) {
	...
	if err != nil {
		...
		ctx.Parent = claims["parent"].(string)
		return ctx, nil
...
//
func GetTokenInfo(v interface{}) (*Context){
	value := reflect.ValueOf(v)
	tokenInfo := &Context{
		ID:       value.Elem().Field(0).String(),
		Username: value.Elem().Field(1).String(),
		Parent:   value.Elem().Field(2).String(),
	}
	return tokenInfo
}